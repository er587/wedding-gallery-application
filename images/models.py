...
class Person(models.Model):
    """People who can be tagged in images"""
    ...
    # Store face encoding for recognition (JSON field for face embeddings)
    face_encoding = models.JSONField(null=True, blank=True, help_text="Face encoding for recognition")
    
    class Meta:
        ordering = ['name']
        unique_together = ['name', 'created_by']  # Allow same name by different users
...
class FaceTag(models.Model):
    """Tags linking faces in images to people"""
    STATUS_CHOICES = [
    ...
    # Face location coordinates (relative to image dimensions)
    ...
    face_encoding = models.JSONField(null=True, blank=True, help_text="Face encoding for this detection")
    ...
    class Meta:
        ordering = ['-created_at']
        unique_together = ['image', 'person', 'face_x', 'face_y']  # Prevent duplicate tags at same location
    
    def __str__(self):
        return f"{self.person.name} in {self.image.title} ({self.status})"
    
    def approve(self, admin_user):
        """Approve this face tag"""
